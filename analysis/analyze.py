...
def tabulate(res, interesting_pairs):
    for (depth, fvc) in interesting_pairs:
        id_res_orig = res[(res["transf"] == "id") & (res["name"] == "orig") & (res["depth"] == depth) & (res["fvc"] == fvc)].drop(["transf", "inv_depth", "name", "fvc"], axis=1).groupby("depth").mean().reset_index()
        id_res_einc = res[(res["transf"] == "id") & (res["name"] == "einc") & (res["depth"] == depth) & (res["fvc"] == fvc)].drop(["transf", "inv_depth", "name", "fvc"], axis=1).groupby("depth").mean().reset_index()
