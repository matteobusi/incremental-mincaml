...
def tabulate(res, interesting_pairs):
    for (num_fact, xi_invalidated) in interesting_pairs:
        id_res_orig = res[(res["name"].str.startswith(orig_n)) & (res["num_fact"] == num_fact) & (res["xi_invalidated"] == xi_invalidated)].drop(["xi_invalidated", "name"], axis=1).groupby("num_fact").mean().reset_index()
        id_res_einc = res[(res["name"].str.startswith(einc_n)) & (res["num_fact"] == num_fact) & (res["xi_invalidated"] == xi_invalidated)].drop(["xi_invalidated", "name"], axis=1).groupby("num_fact").mean().reset_index()
...
def plot_on_pdf (filename, res):
    for num_fact in res["num_fact"].unique():
