...
def tabulate(res, interesting_pairs):
    for (depth, fvc) in interesting_pairs:
        id_res_orig = res[(res["name"] == orig_n) & (res["depth"] == depth) & (res["fvc"] == fvc)].drop(["inv_depth", "name", "fvc"], axis=1).groupby("depth").mean().reset_index()
        id_res_einc = res[(res["name"] == einc_n) & (res["depth"] == depth) & (res["fvc"] == fvc)].drop(["inv_depth", "name", "fvc"], axis=1).groupby("depth").mean().reset_index()
