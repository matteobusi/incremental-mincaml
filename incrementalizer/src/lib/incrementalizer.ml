...
struct
    ...
    let rec typing ?wcache c gamma t =
        let wc = (match wcache with None -> c | Some wc -> wc) in
        ...
        let ts = L.get_sorted_children t in
            ...
            | [] -> (* Call the original algorithm and update the cache *)
                (let r = OriginalFunAlgorithm.typing gamma t in
                    Cache.replace wc hash (gamma, r); r)
            | _ -> (* This is the inductive case, either a hit or a miss *)
                ...
                | None -> (* This is a miss *)
                    ...
                    *)
                    let rs = List.fold_left (fun rs (i, ti) -> rs@[typing ~wcache:wc c (L.tr i ti t gamma rs) ti]) [] ts in
                        ...
                        | Some res -> Cache.replace wc hash (gamma, res); res))
                | Some res -> res)
end
