(* File: benchmark.ml
   ...
   *********************************************************************
   ...
   Forked by Matteo Busi.
   Changes:
    - Added setup function to throughput
*)
...
(* [runloop n_iters n setup f] returns the elapsed time of running [n >= 0L] times [f] with the argument [x].
   The structure returned declare [n_iter] iterations. *)
let runloop n_iters n f setup =
  ...
  for _ = 1 to n1 do
    for _ = 0 to max_int do ignore(f (setup ())) done; (* [max_iter] runs *)
  done;
  for _ = 1 to n0 do ignore(f (setup ())) done;
  let t1 = make n_iters in
    pos_sub t1 !t0

(* time a null-loop; no iter count *)
let null_loop n = runloop 0L n ignore (fun () -> ())

let setup_loop n setup = runloop 0L n setup (fun () -> ())
...
let timeit n f setup =
  ...
  let tmp = pos_sub bm bs in (* approx: removes the setup time from total *)
    ...
    bn.wall +. bm.wall +. bs.wall  (* how much the user had to wait *)
...
let throughput tmin out ?min_count ?min_cpu ~style ?fwidth ?fdigits
  ...
  let rec run_test nmin niter bm_init total_wall =
    let bm, wall = timeit niter f setup in
  ...
  let rec estimate_niter nmin n tn wall =
    if tn >= tpra then
      ...
      let new_bn, new_wall = timeit new_n f setup in
  ...
  let rec min_iter n ~takes_long:previous_took_long total_wall =
    ...
    let bm, wall = timeit n f setup in
    let tn = cpu_process bm in
    let total_wall = total_wall +. wall in
    if tn < 0.1 then (
