...
let typecheck cache env (e : int Annotast.t) =
  let rec typecheck_aux cache env (e : (int * VarSet.t) Annotast.t) =
    let compat_env env envp e =
      ...
      *)
      M.equal (=) env envp
    and cache_res = (Cache.extract_cache (fst (Annotast.get_annot e)) cache) in
    ...
    | Some (gamma_cand, tau_cand) when not (compat_env env gamma_cand e) -> (* In cache but context is not valid *)
      ...
      cache_miss_itc env cache e
    | Some (gamma_cand, tau_cand) -> tau_cand
  and cache_miss_itc gammao cache e = let gamma = env (*M.restrict gammao (snd (Annotast.get_annot e))*) in (* Calling this produces a type and MODIFIES cache *)
    match e with
    | Unit((hash, _)) -> Type.Unit
    | Bool(_, (hash, _)) -> Type.Bool
    | Int(_, (hash, _)) -> Type.Int
    | Float(_, (hash, _)) -> Type.Float
    | Var(x, (hash, _)) -> let tau = Typing.extract_type (Typing.typecheck gamma e) in Cache.replace cache hash (gamma, tau);  tau
    | Not(e1, (hash, _)) -> 
      let tau_e1 = typecheck_aux cache gamma e1 in 
    ...
    | IBop(_, e1, e2, (hash, _)) ->
      let tau_e1 = typecheck_aux cache gamma e1 in 
      let tau_e2 = typecheck_aux cache gamma e2 in 
      Typing.check Type.Int tau_e1; 
    ...
    | If(e1, e2, e3, (hash, _)) ->
      let tau_e1 = typecheck_aux cache gamma e1 in 
      let tau_e2 = typecheck_aux cache gamma e2 in 
      let tau_e3 = typecheck_aux cache gamma e3 in 
      Typing.check Type.Bool tau_e1;
    ...
    | Let(x, e1, e2, (hash, _)) -> 
      let tau_e1 = typecheck_aux cache gamma e1 in 
      let tau_e2 = typecheck_aux cache (M.add x tau_e1 gamma) e2 in 
      Cache.replace cache hash (gamma, tau_e2);
      tau_e2
    | LetRec ({ name = (x, t); args = yts; body = e1 }, e2, (hash, _)) ->
      let gamma_rec = M.add x (Type.Fun(List.map snd yts, t)) (M.add_list yts gamma) in
      let tau_e1 = typecheck_aux cache gamma_rec e1 in 
      let tau_e2 = typecheck_aux cache gamma_rec e2 in 
      Typing.check tau_e1 t;
      Cache.replace cache hash (gamma, tau_e2);
      tau_e2
    | App (e1, e2, (hash, _)) -> 
      let tau_e1 = typecheck_aux cache gamma e1 in 
      let itc_e2 = List.map (fun yi -> typecheck_aux cache gamma yi) e2 in 
      let Type.Fun(ts, tr) = tau_e1 in
      Typing.check tau_e1 (Type.Fun(itc_e2, tr));
      Cache.replace cache hash (gamma, tr);
      tr
    | Tuple(es, (hash, _)) ->
      let tes = List.map (fun yi -> typecheck_aux cache gamma yi) es in 
    ...
    | LetTuple(x, e1, e2, (hash, _)) ->
      ...
      then
        ...
        let tau_e2 = typecheck_aux cache (M.add_list2 x ts gamma) e2 in 
        Cache.replace cache hash (gamma, tau_e2);
        tau_e2
    | Array(e1, e2, (hash, _)) ->
      let tau_e1 = typecheck_aux cache gamma e1 in 
    ...
    | Put (e1, e2, e3, (hash, _)) ->
      ...
      Type.Unit
  in typecheck_aux cache env (Annotast.free_variables e)
end