...
(*
    Modified from Core_bench to include a setup function before _each_ run of the benchmark
*)
...
let measure =
  ...
  let module M  = Core_bench.Measurement_sample in
  fun run_config test ->
    (* test function *)
    let Stest.Basic_test.T f = test.Stest.Basic_test.f in
    let f = f `init in

    (* setup function *)
    let Stest.Basic_test.S setup = test.Stest.Basic_test.setup in
    ...
    let measurement =
      Measurement.create
        ~name:(Stest.Basic_test.name test)
        ~test_name:(Stest.Basic_test.test_name test)
        ~file_name:(Stest.Basic_test.file_name test)
        ~module_name:(Stest.Basic_test.module_name test)
        ~largest_run
        ~sample_count:total_samples
        ~samples:results
    in
    Verbosity.print_high "%s: Total time taken %s (%d samples, max runs %d).\n%!"
      (Stest.Basic_test.name test)
...
let measure_all
  ...
  if (RC.fork_each_benchmark run_config) then
    ...
    List.map tests ~f:(fun test -> measure run_config test)
